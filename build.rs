...
fn main() {
    ...
    let app_path = bin_output_dir.join("wasmedge-hyper-server.wasm");
    let layer_path = out_dir.join("layer.tar");
    tar::Builder::new(File::create(&layer_path).unwrap())
        .append_path_with_name(&app_path, "wasmedge-hyper-server.wasm")
        .unwrap();
    ...
    let config = spec::ConfigBuilder::default()
        .entrypoint(vec!["/wasmedge-hyper-server.wasm".to_owned()])
        .build()
        .unwrap();
    ...
    builder.add_config(
        img,
        "ghcr.io/containerd/runwasi/wasmedge-hyper-server:latest".to_string(),
    );
